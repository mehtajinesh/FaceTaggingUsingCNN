    ...
    for imagePath, faces in imageFaces.iteritems():
        ...
        try:
            ...
            for item in faces:
                ...
                imageName = item.split("\t")[1].replace(".jpg", "").strip()
                #print imageName
                ...
                for j in range(0, im.size[1] - face_height + 1, step):
                    ...
                    for face_region in face_regions:
                        ...
                        if(IoU >= 0.5):
                            #print "Hi"
                            crop = im.crop((i,j, i + face_width, j+face_height))
                            crop.save(path+"/crop_images/Face/" + imageName + "_" + str(count) + ".jpg")
                            output.write(path+"/crop_images/Face/" + imageName + "_" + str(count) + ".jpg" + " " + str(IoU) + "\n")
                            output1.write(imageName + "_" + str(count) + ".jpg" + " " + str(1) + "\n")
                            #print "Hi2"
                            count += 1
                            iou_face = True
                    if(iou_face):
                            ...
                            if(IoUCount == len(face_regions)):
                                ...
                                if(type(array.std(axis=0)) is np.float64):
                                    if(array.std(axis=0) < 8 and random.random() < 0.05):
                                        crop = im.crop((i,j, i + face_width, j+face_height))
                                        crop.save(path+"/crop_images/Non-face/" + imageName + "_" + str(count) + ".jpg")
                                        output.write(path+"/crop_images/Non-face/" + imageName + "_" + str(count) + ".jpg" + " " + str(IoU) + "\n")
                                        output1.write(imageName + "_" + str(count) + ".jpg" + " " + str(0) + "\n")
                                        count += 1
                                        continue
