...
def convert_full_conv():
 # Load the original network and extract the fully connected layers' parameters.
    ...
    net_full_conv = caffe.Net('/media/jinesh/ACB6A202B6A1CCE0/Academics/8thSemester/CNN/prototxt_files/face_full_conv.prototxt',
                           '/media/jinesh/ACB6A202B6A1CCE0/Academics/8thSemester/CNN/Models/alexNet__iter_60000.caffemodel',
                              caffe.TEST)
...
def face_detection(imgList):
	...
	for imgFile in open(imgList).readlines():
		scales = []
		print(imgFile)
		#initialize a factor for scaling
		factor = 0.793700526
		#open Image
		img = Image.open(imgFile.strip())
		min = 0
		max = 0
		#identify if width is more or depth is more
		if(img.size[0] > img.size[1]):
			...
			min = img.size[0]
			max = img.size[1]
			#use delimiter for scaling
		print "min : " + str(min) + " , max : " + str(max)
		delim = 2500/max
		print delim
		#check if image is 2500 pixels (else scale down to )
		if(delim == 1):
			scales.append(1)
		elif(delim > 1):
			scales.append(delim)
		...
		for scale in scales:
			...
			scale_img.save("/media/jinesh/ACB6A202B6A1CCE0/Academics/8thSemester/CNN/tmp.jpg")
			...
			im = caffe.io.load_image("/media/jinesh/ACB6A202B6A1CCE0/Academics/8thSemester/CNN/tmp.jpg")
...
if __name__ == "__main__":
	#convert all the fully connected layer to convolutional layers
    convert_full_conv()
